...
directory="/usr/share/icons/Paper/48x48/"
...
while IFS= read -r i; do
    if [[ -f "$i" ]]; then
        ...
        incomplete_icon_path=$(find "$directory" "$icon" | head -1)
...
while IFS= read -r a; do
    app_spaces=$(printf %-60.60s "$a" | sed 's/\([^ ]\)* +/\1	/g')
    printf %\*.60s -60 a 
...
selection=$(echo -ne "$final_rofi" | rofi -markup-rows -theme /home/antoine/.config/rofi/themes/notifications.rasi -dmenu -eh 3 -a "$active_list" -u "$urgent_list" -sep '\x0f' -p "Notification Center" -no-fixed-num-lines -lines 8 -i -no-config)
